<?php
    ...
    //비밀번호 수정에 필요한 정보있는지 체크
    $user_need_info = array("JWT","password","change_password");
    ...
    while ( $row = $now_user->fetch( PDO::FETCH_ASSOC ) ){  
     ...
     $data_arr["password"]=$data_arr["change_password"];
    //비밀번호 수정
    if($user->change_password($data_arr)){
        header("HTTP/1.1 200");
        echo(json_encode(array("message"=> "password modified")));
    }else{
        header("HTTP/1.1 400");
        echo(json_encode(array("message"=>"no user")));
    }
