<?php
    ...
    //유저수정에 필요한 정보있는지 체크
    $user_need_info = array("JWT","id","password","email","address","phone_number","home_number");
    ...
    //유저 정보 수정
    if($user->modify_user($data_arr)){
        header("HTTP/1.1 200");
        echo(json_encode(array("message"=> "account modified")));
    }else{
        header("HTTP/1.1 400");
        echo(json_encode(array("message"=>"no user")));
    }
