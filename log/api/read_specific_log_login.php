<?php
    ...
    if(!isset($data_arr["search_by_id"])){
        $data_arr["search_by_id"]="";
    }
    else{
        //sql을 직접 이런식으로 삽입하는것보다 좋은 방법 생각해보기
        $data_arr["search_by_id"]=$log_login->user_table.".id = ".$data_arr["serach_by_id"];
    }
    $result = $log_login->read_specific($data_arr);
    $num=$result->rowCount();
    //응답 json변환
    if($num>0){
        header("HTTP/1.1 200");
