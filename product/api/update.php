<?php
    ...
    //제품수정에 필요한 정보있는지 체크
    $user_need_info = array("JWT","_id","name","weight","price","category","detail");
    ...
    //제품 수정
    if($product->modify($data_arr)){
        header("HTTP/1.1 200");
        echo(json_encode(array("message"=> "produt modified")));
    }else{
        header("HTTP/1.1 400");
        echo(json_encode(array("message"=>"no such product")));
    }
